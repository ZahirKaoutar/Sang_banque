    ...
    <div class="max-w-6xl mx-auto space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-white border border-gray-100 rounded-2xl p-4 text-center">
                ...
                <p id="resultsCount" class="text-3xl font-black text-blue-600 mt-2">{{ $totalHopitaux }}</p>
            </div>
        </div>
        ...
        @if($hopitaux->isEmpty())
            <div class="bg-white border border-gray-100 rounded-3xl p-12 text-center text-gray-400">
                ...
                <p class="font-bold text-lg">Aucun hôpital</p>
                <p class="text-sm">Créez votre premier hôpital</p>
            </div>
        @else
            <div id="hopitauxGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($hopitaux as $hopital)
                    <div class="hopital-card bg-white border border-gray-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition flex flex-col gap-4"
                         data-name="{{ strtolower($hopital->name) }}"
                         data-city="{{ strtolower($hopital->city) }}"
                         data-adress="{{ strtolower($hopital->adress) }}">

                        <div class="flex items-start justify-between">
                            <div>
                                <h3 class="text-lg font-bold text-dark">{{ $hopital->name }}</h3>
                                <p class="text-sm text-gray-500 mt-1">📍 {{ $hopital->city }}</p>
                            </div>
                            <form action="{{ route('admin.delete-hopital', $hopital->id) }}" method="POST"
                                onsubmit="return confirm('Supprimer {{ addslashes($hopital->name) }} ?')">
                                @csrf
                                @method('DELETE')
                        ...
                        <div class="space-y-1 text-sm text-gray-600">
                            <p>📬 {{ $hopital->adress }}</p>
                            <p>🪪 {{ $hopital->liscence_number }}</p>
                        </div>

                        <a href="{{ route('admin.show-hopital', $hopital->id) }}"
            ...
            <p id="emptySearch" class="hidden text-center text-gray-400 py-8">🔍 Aucun hôpital trouvé</p>
        @endif
    </div>
</div>
...
<script>
    ...
    const cards = document.querySelectorAll('.hopital-card');
