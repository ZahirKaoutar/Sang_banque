...
@section('title', 'Créer Hôpital - HemoLife')

@section('content')
<div class="py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-2xl mx-auto">
        <div>
            <h1 class="text-4xl font-bold text-dark mb-2">Créer un Hôpital</h1>
            <p class="text-gray-600">Créez un nouveau hôpital et son agent</p>
        </div>

        <form method="POST" action="{{ route('admin.store-hopital') }}" class="mt-8 bg-white rounded-3xl p-8 border border-gray-100 space-y-8">
            @csrf
            ...
            <div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="hospital_name" value="{{ old('hospital_name') }}" placeholder="Hôpital X"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-red-mid focus:border-transparent @error('hospital_name') border-red-500 @enderror" />
                        @error('hospital_name')
                            <p class="text-red-deep text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="adress" value="{{ old('adress') }}" placeholder="456 Avenue de la Santé"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-red-mid focus:border-transparent @error('adress') border-red-500 @enderror" />
                        @error('adress')
                            <p class="text-red-deep text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('admin.hopitaux') }}" class="flex-1 py-4 bg-gray-100 text-dark rounded-2xl font-bold hover:bg-gray-200 transition text-center">
                    Annuler
                </a>
            </div>
        </form>
