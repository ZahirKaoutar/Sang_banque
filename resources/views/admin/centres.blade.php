...
<div class="py-12 px-4 sm:px-6 lg:px-8" x-data="{ search: '' }">
    <div class="max-w-6xl mx-auto space-y-8">
        ...
        @if($centres->isNotEmpty())
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-white border border-gray-100 rounded-2xl p-4 text-center">
                    <p class="text-gray-400 text-xs uppercase font-bold">Total des centres</p>
                ...
                <div class="bg-white border border-gray-100 rounded-2xl p-4 text-center">
                    ...
                    <p class="text-3xl font-black text-blue-600 mt-2" x-text="filteredCentres.length">0</p>
                </div>
            </div>
        @endif

        <!-- Empty State -->
        <template x-if="filteredCentres.length === 0 && search !== ''">
                    ...
                    <div class="bg-white border border-gray-100 rounded-3xl p-6 shadow-sm hover:shadow-md transition">
                        ...
                        <div class="flex items-start justify-between mb-4">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-500 mt-1">📍 <span x-text="centre.city"></span></p>
                            </div>
                            <form :action="`/admin/centres/${centre.id}`" method="POST" onsubmit="return confirm('Êtes-vous sûr?');" style="display: inline;">
                                @csrf
                                @method('DELETE')
                        ...
                        <div class="space-y-2 text-sm text-gray-600 mb-4">
                            <p>📬 <span x-text="centre.adress"></span></p>
                            <p>🪪 <span x-text="centre.liscence_number"></span></p>
                        </div>

                        <a :href="`/profile/${centre.user_id}`" class="w-full px-4 py-2 text-center bg-red-pale text-red-mid rounded-lg hover:bg-red-light hover:text-white transition font-bold">
                            Voir Agent
                        </a>
                    </div>
                </template>
        ...
        @if($centres->isEmpty())
            <div class="bg-white border border-gray-100 rounded-3xl p-8">
                @include('components.empty-state', ['icon' => '🏥', 'title' => 'Aucun centre', 'subtitle' => 'Créez votre premier centre de transfusion'])
            </div>
        @endif
    </div>
