...
@section('title', $hopital->name . ' - HemoLife')

@section('content')
<div class="py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto space-y-6">

        <a href="{{ route('admin.hopitaux') }}" class="inline-flex items-center gap-2 text-gray-500 hover:text-red-mid transition text-sm">
        ...
        <div class="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
            <div class="flex items-start justify-between mb-6">
                <div>
                    <h1 class="text-3xl font-bold text-dark">{{ $hopital->name }}</h1>
                    <p class="text-gray-500 mt-1">📍 {{ $hopital->city }}</p>
                </div>
                <div class="flex gap-2">
                    <a href="{{ route('admin.edit-hopital', $hopital->id) }}"
                        class="px-4 py-2 bg-blue-100 text-blue-700 rounded-xl hover:bg-blue-200 transition font-bold text-sm">
                        Modifier
                    </a>
                    <form action="{{ route('admin.delete-hopital', $hopital->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-gray-50 rounded-2xl p-4">
                    ...
                    <p class="text-dark font-medium">{{ $hopital->adress }}</p>
                </div>
                <div class="bg-gray-50 rounded-2xl p-4">
                    <p class="text-xs text-gray-400 uppercase font-bold mb-1">Ville</p>
                    <p class="text-dark font-medium">{{ $hopital->city }}</p>
                </div>
                <div class="bg-gray-50 rounded-2xl p-4">
                    <p class="text-xs text-gray-400 uppercase font-bold mb-1">Numéro de licence</p>
                    <p class="text-dark font-medium">{{ $hopital->liscence_number }}</p>
                </div>
                <div class="bg-gray-50 rounded-2xl p-4">
                    <p class="text-xs text-gray-400 uppercase font-bold mb-1">Créé le</p>
                    <p class="text-dark font-medium">{{ $hopital->created_at->format('d/m/Y') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
            ...
            <div class="flex items-center gap-4 mb-6">
                ...
                <div class="ml-auto flex items-center gap-2">
                    ...
                    @else
                        ...
                        <form method="POST" action="{{ route('admin.ban-user', $hopital->user->id) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-gray-50 rounded-2xl p-4">
                    ...
                    <p class="text-dark font-medium">{{ $hopital->user->email }}</p>
                </div>
                <div class="bg-gray-50 rounded-2xl p-4">
                    <p class="text-xs text-gray-400 uppercase font-bold mb-1">Téléphone</p>
                    <p class="text-dark font-medium">{{ $hopital->user->phone ?? '-' }}</p>
                </div>
                <div class="bg-gray-50 rounded-2xl p-4">
                    <p class="text-xs text-gray-400 uppercase font-bold mb-1">Ville</p>
                    <p class="text-dark font-medium">{{ $hopital->user->city ?? '-' }}</p>
                </div>
                <div class="bg-gray-50 rounded-2xl p-4">
                    <p class="text-xs text-gray-400 uppercase font-bold mb-1">Membre depuis</p>
                    <p class="text-dark font-medium">{{ $hopital->user->created_at->format('d/m/Y') }}</p>
                </div>
            </div>
        </div>
        @endif
